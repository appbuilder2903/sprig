...
// NEW FEATURE 1: Score System
let score = 0
let zombiesKilled = 0
let blocksMined = 0

// NEW FEATURE 2: Torch Placement & Light Radius
let placedTorches = []
...
// NEW FEATURE 3: Show HUD with health, score, and inventory
function showHUD() {
  addText(`HP:${health}/${maxHealth} Score:${score}`, {
    x: 1,
    y: 14,
    color: health > 5 ? color`3` : color`4`
  })
  
  // Show current block and count
...
function attackEntityTile(x, y) {
  ...
  // NEW: Track zombie kills and score
  if (tile.some(s => s.type === zombie)) {
    zombiesKilled++
    score += 10
    playTune(killer)
  }
  if (tile.some(s => s.type === squirrel)) {
    score += 2
  }
...
function placeBlockAt(x, y) {
  ...
  // NEW FEATURE 2: Track torch placement
  if (type === torch) {
    placedTorches.push({x, y})
  }
...
function spawnSquirrels() {
  for (let i = 0; i < 2; i++) {  // NEW: Spawn 2 squirrels instead of 1
...
function spawnNightZombies() {
  const count = 3 + Math.floor(score / 50)  // NEW: More zombies as score increases
...
function damagePlayer(amount) {
  ...
  // NEW: Screen shake effect via text
  addText("OUCH!", { x: 8, y: 7, color: color`4` })
  setTimeout(() => clearText(), 300)
  
  if (health <= 0) {
    playTune(deathSound)
    addText("You Died! Score:" + score, { x: 2, y: 0, color: color`4` });
    setTimeout(() => {
      clearText();
    }, 10000);
    respawnAfterDeath()
...
// Level definitions (keeping existing ones)
const levels1 = [
  map`
..........
..........
...
function respawnAfterDeath() {
  // NEW FEATURE 4: Lose some score on death
...
// NEW FEATURE 5: Health pack pickup
onInput("h", () => {
  const p = getPlayer()
  if (!p) return
  
  // Check if player has enough blocks to craft health
  if (inventory[dirt] >= 5) {
    inventory[dirt] -= 5
    health = Math.min(maxHealth, health + 3)
    playTune(pickupSound)
    addText("Healed +3!", { x: 7, y: 7, color: color`3` })
    setTimeout(() => clearText(), 1000)
  } else {
    addText("Need 5 dirt!", { x: 7, y: 7, color: color`4` })
    setTimeout(() => clearText(), 1000)
  }
})

// NEW: Show stats periodically
setInterval(() => {
  clearText()
  showHUD()
}, 2000)
...
setInterval(checkLavaDamage, 500)